...
| [`13-footprint-economy.md`](13-footprint-economy.md) | **발자국 경제(v2 — 특허 기반 확정).** 코어=KR 256/391의 순서가중 허브/권위(수학식10·청구항3 상속). 이중 평판 통화(저자=권위/발자국자=허브)로 50% 분할·f(L/M)·무료/비용 knob 해소. 우리 확장=두루·규정타석·정정환수. 잔여 해소: 자기발자국=노트 24 §2 / mode=harmonic 확정(결정로그). |
...
| [`21-scale-simulation.md`](21-scale-simulation.md) | **대규모 페르소나 시뮬레이션(2026-07).** 12페르소나·603명·2,606호출(`scripts/sim_scale.py`). ✅ 재사용 82%·시빌 차단·그룹 격리·부하 p95<20ms / 실측 발견 3: ① 교정 역전=고안목 갈아타기 필요(→재검 넛지) ② **스텁 파밍 뚫림(허브 105 vs 28)** ③ 생성 쿼터 부재. 백로그 6건 우선순위 확정. |
| [`22-integrity-fixes.md`](22-integrity-fixes.md) | **무결성·비용 수리 묶음(노트 21 백로그 전체 구현).** ① 스텁 링크 안목 적립 차단(허브 105→0 실측) ② 링 상호 상속 1/\|무리\| 감액(`kin`) ③ 생성 쿼터(`NIGHTWISH_ASK_QUOTA`, 429) ④ ⚔️정정 제기 배지+프로필 🔎재검 ⑤ 조건 패싯(AI후보→탭확정, pgstore 컬럼) ⑥ 종합 근거=렌즈 권위 정렬 ⑦ fork 반증 유도+「대립」 자동 기록. 테스트 164. |
| [`23-ladder-overhaul.md`](23-ladder-overhaul.md) | **대대적 개편 — 비용 사다리가 곧 화면이다.** 재사용→종합→생성 사다리를 단일 동선 UI(🔎기존 답 찾기 주버튼·재사용 리드 배너·카드 신호 강화·✨생성은 마지막 계단+쿼터 표시)와 티어드 라우팅(`set_cheap_ai` — 종합·요약·분류=싼 층/생성=프런티어, 노트 20 축 E 해소)으로. 검색 캡(24). 테스트 167. |
| [`24-backlog-complete.md`](24-backlog-complete.md) | **잔여 백로그 완결 — 결정-미구현 0.** BYOK-lite(429→키 입력, localStorage만·서버 무저장) · 자동 자기발자국(사람 저작만 가중1, AI 초안 제외 — 노트 13 잔여 해소) · ✅아직 유효함(재검 판정 1탭=시계 리셋) · 모바일 사다리 압축. 테스트 170. 남은 건 코드가 아니라 **실사용 한 바퀴**. |
| [`25-hybrid-groups.md`](25-hybrid-groups.md) | **하이브리드 그룹 — 사일로+렌즈 (노트 05 개정).** 그룹 공간=우리가 만든 것+발자국으로 들여온 것(새 그룹=빈 화면), 공용은 🌐 명시적 계단, **그룹 발화=그룹 소유(기밀 기본, `그룹::슬러그`)**·공개는 publish로만. `_visible`(읽기)/`in_space`(소속) 분리. 불변: 그룹코인 단방향·[[참조]]=공용 앵커. 테스트 174+E2E. |
