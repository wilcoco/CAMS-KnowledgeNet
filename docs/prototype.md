...
├── economy.py     닫힌 포인트 경제 (UBI·스테이킹·배당 게이트·에스크로·소각)
...
| 설계 개념 | 왜 미구현 | 어디서 |
|----------|----------|--------|
| 외부 현실 닻(제조 수율 판정) | MVP는 인메모리, 실 측정 없음 | 로드맵 P0 |
| 배당 시간붕괴 / 기여 갱신 요구 | 자본 자기증식 방어(비평 §1.3) | 로드맵 P1 / critique |
| 잠복 포인트 회수 vs 가지 보존 분리 | 인플레-잠김 딜레마(비평 §1.2) | 로드맵 P1 / critique |
| 거버넌스 자동 분권 사전공약 | 부트스트랩 권력집중(비평 §3) | 로드맵 P1 / critique |
| 실제 검색(임베딩)·실제 LLM | stub로 주입 (`pipeline`의 `*_fn`) | 로드맵 P3 |
| 시빌/담합 저항 | 정체성 계층 부재 | 로드맵 P3 |
