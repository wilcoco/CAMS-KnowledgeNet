...
def main() -> None:  # noqa: PLR0915 — 시나리오 스크립트
    ...
    report.append((
        ...
        f"개선점: ① 정정 제기 시 원답 지지자에게 재검 넛지(사람이 재판단), "
        f"② 18-P3 '왜 틀렸나' 플레이스홀더 미구현"))
    ...
    report.append((
        ...
        f"두루 {eff:.1f}표로 접힘(√n), 렌즈 순위 {rank or '권외'}위. 잔여 위험: 링 상호 허브 적립단은 "
        f"미할인(결정로그 미해결 1) — 표적 권위엔 못 미치나 안목 보드 오염 가능"))
    ...
    report.append((
        ...
        f"개선점: 스텁 대상 링크의 허브 적립 제외/감액 (결정로그 미해결 2)"))
    ...
    report.append((
        ...
        "⚠️",
        f"초안 {spam_drafts}건 전부 생성됨(고유 질문이라 dedup 불가). 초안 배지로 "
        f"구분되고 발자국 없으면 랭킹에 안 오르나, **생성 쿼터가 없어** 실LLM이면 "
        f"비용 무방비 — 개선점: 노트 19 무료 쿼터(하루 N건) 구현이 방어선"))
    ...
    report.append((
        ...
        f"개선점: 신선도 가중이 근거 *정렬*에 아직 미반영(18-P0 잔여 절반)"))
