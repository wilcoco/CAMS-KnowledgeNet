...
자체가 다음 질문의 답이 되어 가치를 만든다. 평가의 보상은 *기여 + 그에 건
확신(포인트)*이며, 인류의 **암묵지(暗默知)를 성과급 유인으로 형식지로 끌어내**
...
이 프로젝트는 두 트랙으로 나누어 진행한다:

- **🔬 이 브랜치 `claude/ontology-knowledge-evaluation-uzuDx` — 설계·풀스펙 검증 트랙 (Single Source of Truth).**
  설계 정본·용어집·비평·로드맵·B2B 검증과 *로직 검증용 참조 구현*이 여기 쌓인다.
  풀 스펙 서비스로 가기 위한 로직을 계속 검증·수립하고 향후 푸시한다.
- **🚀 MVP 트랙 — 빠른 현실 검증.**
  가볍고 배포 가능한 MVP로 시장/현실(특히 **B2B 한 팀·한 불량**)에서 닻이 도는지
  *실제로* 증명한다. 이 트랙은 위 SoT의 설계를 따르고, 검증된 로직만 가져다 쓴다.

> 원칙: **설계는 이 트랙에 모은다(SoT). MVP는 그것을 구현한다.** 코드가 두 군데로
> 갈라질 수 있으므로, 이 트랙의 코드는 "참조 구현"으로 본다.
